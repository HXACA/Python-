...
def work(videoStr,tk=-1):
    ...
    videoWriter.videoWriter(bestans,videoStr,bestClust,bestCentroids,imgList)
    t6 = time.clock()
...
if __name__ == '__main__':
    videos = os.listdir('train/')
    for video in videos:
        videoPath = os.path.join('train/',video)
        work(videoPath,6)
        try:
